...
- [Packages `@nextgis/[package-name]`](#packages-nextgispackage-name)
  ...
  - [QuickMapServices integration](#quickmapservices-integration)
  - [Utilities](#utilities)
...
| --- | --- |
| [`@nextgis/ngw-leaflet`](packages/ngw-leaflet) | Leaflet |
| [`@nextgis/ngw-ol`](packages/ngw-ol) | OpenLayers |
| [`@nextgis/ngw-maplibre-gl`](packages/ngw-maplibre-gl) | MapLibre GL JS |

All three provide the common `NgwMap` API. Use
[`@nextgis/ngw-connector`](packages/ngw-connector) for NextGIS Web HTTP API
requests without a map, and [`@nextgis/ngw-kit`](packages/ngw-kit) for
NextGIS Web map adapters and resource utilities.

React applications can use the matching `react-ngw-*` component. Applications
that already have a native Leaflet, OpenLayers, or MapLibre GL JS map can use
the matching `qms-*` package to add services from NextGIS QMS.

See the [package architecture and selection guide](docs/PACKAGES.md) for the
relationship between public packages, `ngw-map`, `webmap`, and map adapters.
...
- [ngw-map](https://github.com/nextgis/nextgis_frontend/tree/master/packages/ngw-map) – common NextGIS Web map class used by the ready-to-use map packages;
...
- [ngw-types-loader](https://github.com/nextgis/nextgis_frontend/tree/master/packages/ngw-types-loader) – installs TypeScript declarations generated by a specific NextGIS Web deployment;
...
Use an engine-specific package with an existing native map. Use `qms-core`
without a map, or `qms-kit` when building on the common `webmap` API:

- [qms-leaflet](https://github.com/nextgis/nextgis_frontend/tree/master/packages/qms-leaflet) – QMS layers and search control for Leaflet;
- [qms-ol](https://github.com/nextgis/nextgis_frontend/tree/master/packages/qms-ol) – QMS layers and search control for OpenLayers;
- [qms-maplibre-gl](https://github.com/nextgis/nextgis_frontend/tree/master/packages/qms-maplibre-gl) – QMS layers and search control for MapLibre GL JS;
- [qms-core](https://github.com/nextgis/nextgis_frontend/tree/master/packages/qms-core) – map-independent QMS client, catalog, and shared control UI;
- [qms-kit](https://github.com/nextgis/nextgis_frontend/tree/master/packages/qms-kit) – QMS layer adapter for `webmap`-based libraries.
...
Below is a basic example using the ready-to-use Leaflet package. Replace the
import with `@nextgis/ngw-ol` or `@nextgis/ngw-maplibre-gl` to use another
rendering engine with the same common API:
...
By default, it fetches declarations from
[demo.nextgis.com](https://demo.nextgis.com). The generated modules are
installed under `node_modules/@nextgisweb` and resolve without changes to
`tsconfig.json`.
