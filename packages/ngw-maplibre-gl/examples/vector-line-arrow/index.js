...
NgwMap.create({
  ...
  const points = /** @type {import('@nextgis/webmap').VectorLayerAdapter} */ (
    ngwMap.getLayer('cafe')
  );
  if (points && points.getLayers) {
    ...
    for (let i = 0; i < features.length; i += 2) {
      const [start, end] = [i, i + 1].map(
        (index) =>
          /** @type {import('geojson').Point} */ (features[index].geometry)
            .coordinates,
      );
      if (start && end) {
...
function getArrowPoints(end, prev, length = 1, deviationAngle = Math.PI / 6) {
  const dx = end[0] - prev[0];
  const dy = end[1] - prev[1];
