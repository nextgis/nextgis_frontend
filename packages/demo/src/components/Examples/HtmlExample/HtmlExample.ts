...
const changeHtmlMapAdapter:
  (html: string, adapter: Package, adapters: Package[]) => string = require('../../../../scripts/changeHtmlMapAdapter');
...
export class HtmlExample extends Vue {
  ...
  private _getDefaultNgwMap(): string {
    ...
    const exist = fromStorage && this.item.ngwMaps && this.item.ngwMaps.find((x) => x.name === fromStorage);
    if (exist) {
      return exist.name;
    }
