...
import { fetchNgwLayerItemExtent, fetchNgwLayerItems } from '@nextgis/ngw-kit';
import NgwMap from '@nextgis/ngw-leaflet';

import type { FeatureItem } from '@nextgis/ngw-connector';
import type { Geometry } from 'geojson';

interface EatHereProperties {
  NAME: string;
}
...
function fillFeatureList() {
  ...
  connector.getResourceIdOrFail(keyname).then((resourceId) => {
    fetchNgwLayerItems<Geometry, EatHereProperties>({
...
function drawHtml(items: FeatureItem<EatHereProperties>[]) {
...
function createItemBlock(item: FeatureItem<EatHereProperties>) {
  const itemBlock = document.createElement('div');
  itemBlock.className = 'item-block';
  itemBlock.innerHTML = item.fields.NAME;
  itemBlock.addEventListener('click', () => {
    return connector.getResourceIdOrFail(keyname).then((resourceId) => {
      ...
      connector
        ...
        .then((resp) => {
          ...
          for (let i = 0; i < items.length; i++) {
            ...
            numPageBlock.innerHTML = String(
              PAGE === Number(p) ? '<strong>' + p + '</strong>' : p,
            );
            paginationBlock.appendChild(numPageBlock);
            if (p !== '...') {
              numPageBlock.addEventListener('click', () => changePage(p));
            }
          }
...
function changePage(pageNum: number | string) {
  PAGE = Number(pageNum);
  fillFeatureList();
}
...
function pagination(current: number, total: number): Array<number | string> {
  const center = [current - 1, current, current + 1],
    filteredCenter: Array<number | string> = center.filter(
      (p) => p > 1 && p < total,
    ),
