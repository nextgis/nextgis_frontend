...
const bounds: LngLatBoundsArray = [-5.94703, 54.58755, -5.92991, 54.594552];
  ...
  return Object.assign(
    {
      ...
      popupOptions: {
        createPopupContent: (
          e: CreatePopupContentProps<Feature<Geometry, FeatureProperties>>,
        ) => {
          const element = document.createElement('div');
          const table = document.createElement('table');
          table.className = 'popup-table';
          element.innerHTML =
            '<div class="popup-header">Feature Information</div>';
          element.appendChild(table);

          return ngwMap.connector.getResource(resourceId).then((item) => {
            if (!item?.feature_layer) {
              throw new Error('Feature layer resource is unavailable');
            }
