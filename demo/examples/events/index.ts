...
import type { NgwMapEvents } from '@nextgis/ngw-leaflet';
...
ngwMap.onLoad().then(() => {
  ...
  function toggleLayers(status: boolean) {
...
const events: (keyof NgwMapEvents)[] = [
  'create',
  'zoomstart',
  'zoom',
  'zoomend',
...
const timeout: Partial<
  Record<keyof NgwMapEvents, ReturnType<typeof setTimeout>>
> = {};
function addListener(event: keyof NgwMapEvents, el: HTMLElement) {
