...
import { degrees2meters, degrees2Radian, meters2degrees } from '@nextgis/utils';
...
type CameraFeature = Feature<Point, CameraProperties>;
...
NgwMap.create({
  ...
  ngwMap.emitter.on('layer:click', (e) => {
    if (e.layer.id === 'cameras') {
      if (e.selected) {
        updateCameraPanel(e.feature as CameraFeature);
      } else {
        updateCameraPanel();
      }
    }
  ...
  const wrapper = cameraPanel.querySelector<HTMLElement>('.camera-wrapper')!;
  const title = cameraPanel.querySelector<HTMLElement>('.camera-title')!;
  const player = videojs(cameraPanel.querySelector('#vid1'));

  async function updateCameraPanel(feature?: CameraFeature) {
    if (feature) {
      try {
  ...
  function cameraSectorPoly(
    feature: CameraFeature,
  ): Feature<Polygon, CameraProperties> {
    const props = feature.properties;
    const xy = degrees2meters(
    ...
    const angle = degrees2Radian(props.angle !== undefined ? props.angle : 60);
    const focalLength =
