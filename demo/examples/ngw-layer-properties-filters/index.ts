...
const isNumberField = (datatype: FeatureLayerFieldDatatype) =>
...
NgwMap.create({
  ...
  const adapterEl = panel.querySelector('#adapter') as HTMLSelectElement;
  const fieldEl = panel.querySelector('#field') as HTMLSelectElement;
  const operatorEl = panel.querySelector('#operator') as HTMLSelectElement;
  const valueEl = panel.querySelector('#value') as HTMLInputElement;
  ...
  function getFilter() {
    ...
    return [[field.keyname, operatorEl.value, value]] as PropertiesFilter;
  }
  ...
  async function createLayer() {
    ...
    layer = await ngwMap.addNgwLayer({
      ...
      adapter: adapterEl.value as NgwLayerAdapterType,
