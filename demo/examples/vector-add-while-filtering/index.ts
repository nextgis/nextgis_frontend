...
NgwMap.create({
  ...
  const filterFunction: DataLayerFilter<ColorFeature> = (e) => {
...
type ColorFeature = Feature<Point, { color: string }>;

function createFeature(color: string) {
  return {
    type: 'Feature',
    geometry: { type: 'Point', coordinates: getRandomPointCoord() },
    properties: { color: color },
  } satisfies ColorFeature;
}
