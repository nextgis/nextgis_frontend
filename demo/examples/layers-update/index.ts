...
NgwMap.create({
  ...
  const resourceId = await getOrCreateResource(connector, vectorLayerKeyName);

  // Get the style ID for the layer or create a default style if none exists
  const styleId = await getOrCreateStyle(connector, resourceId);
  ...
  function addFeature(coord: LngLatArray) {
  ...
  async function getOrCreateResource(connector: NgwConnector, keyName: string) {
    const res = await connector.getResource(keyName);
    if (!res) {
